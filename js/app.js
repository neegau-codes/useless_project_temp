...
import { RealDetectionService } from './realDetectionService.js';
...
class App {
  constructor() {
    ...
    this.detectionService = new RealDetectionService(() => this.videoEl);
  ...
  bindEvents() {
    ...
    if (this.scenarioToggleBtn) {
      this.scenarioToggleBtn.addEventListener('click', () => {
        if (typeof this.detectionService.setScenario === 'function') {
